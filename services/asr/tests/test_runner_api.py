...
def test_call_worker_for_registered_models(monkeypatch, model_key):
    ...
    def fake_run(cmd, input, capture_output, cwd, check):  # noqa: ANN001
        ...
        return type(
            ...
            {
                "returncode": 0,
                "stdout": json.dumps({"raw": {"segments": []}, "aligned": {"segments": []}}).encode("utf-8"),
                "stderr": b"",
            },
        )()
    ...
    result = runner_api.call_worker(model_key, payload, ASRResultWrapper, runner_index=0)
